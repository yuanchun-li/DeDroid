...
def runTrain(dataset_dir, dataList, output_dir, path_to_androidjar,
             path_to_unuglifyDEX, path_to_nice2predict, featurePara):
    # clear
    ...
    for apk in apkList:
        ...
        train_cmd = "java -jar %s -i %s -o %s -train %s %s" % \
                    (path_to_unuglifyDEX, apk, output_dir, sdkPara, featurePara)

        (retVal, dexLog) = commands.getstatusoutput(train_cmd)
        if retVal != 0:
            print dexLog
            continue
...
def run(dataset_dir, output_dir,
    ...
    iLoop_str = iLoop
    print "pass: %s\ntrain: %s\npredict: %s\n" % \
        (iLoop_str, ",".join(dataList[:-predictLen]), ",".join(dataList[-predictLen:]))
    ...
    featureParaFile = open("%s/feature_options.txt" % currentPassPath, 'w')
    featureParaFile.write(featurePara)
    featureParaFile.close()

    runTrain(dataset_dir, dataList[:-predictLen], currentPassPath, path_to_androidjar,
             path_to_unuglifyDEX, path_to_nice2predict, featurePara)

    runPredict(dataset_dir, dataList[-predictLen:], currentPassPath, path_to_androidjar,
               path_to_unuglifyDEX, path_to_nice2predict, featurePara)
